...
;var app=angular.module("task-manager",["ngRoute"]);;app.config(["$routeProvider",function($routerProvider){$routerProvider.when("/login",{templateUrl:"./partials/login-partial.html",controller:"LoginCtrl"}).when("/tableros",{templateUrl:"./partials/tablero-partial.html",controller:"TableroCtrl"}).when("/tableros/:idTablero/listas",{templateUrl:"./partials/listas-partial.html",controller:"ListasCtrl"}).otherwise({redirectTo:"/login"})}]);;app.controller("ListasCtrl",["$scope","$routeParams","ListaSrv",function($scope,$routeParams,ListaSrv){$scope.listas=[],ListaSrv.listar($routeParams).then(function(response){$scope.listas=response.data},function(response){console.log(response)})}]);;app.controller("LoginCtrl",["$scope",function($scope){$scope.something={}}]);;app.controller("TableroCtrl",["$scope","TableroSrv",function($scope,tableroSrv){$scope.tableros={},tableroSrv.listar().then(function(response){$scope.tableros=response.data},function(response){console.log(response)})}]);;app.service("ListaSrv",["$http",function($http){return{recurso:"/api/listas",listar:function(parms){return $http.get(this.recurso,{params:parms})}}}]);;app.service("TableroSrv",["$http",function($http){return{recurso:"/api/tableros",listar:function(parms){return $http.get(this.recurso,{params:parms})}}}]);