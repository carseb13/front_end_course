...
;var app=angular.module("task-manager",["ngRoute"]);;app.config(["$routeProvider",function($routerProvider){$routerProvider.when("/login",{templateUrl:"./partials/login-partial.html",controller:"LoginCtrl"}).when("/tableros",{templateUrl:"./partials/tablero-partial.html",controller:"TableroCtrl"}).when("/tableros/:id/listas",{templateUrl:"./partials/listas-partial.html",controller:"ListasCtrl"}).otherwise({redirectTo:"/login"})}]);;app.controller("ListasCtrl",["$scope",function($scope){}]);;app.controller("LoginCtrl",["$scope",function($scope){$scope.something={}}]);;app.controller("TableroCtrl",["$scope","TableroSrv",function($scope,tableroSrv){tableroSrv.listar().then(function(response){console.log(response)},function(response){console.log(response)})}]);;app.service("TableroSrv",["$http",function($http){return{recurso:"/api/tableros",listar:function(parms){return $http.get(this.recurso,{params:parms})}}}]);